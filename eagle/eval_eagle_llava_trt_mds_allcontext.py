...
def decode_regular(self, small,
    ...
    for step in range(0, self.max_new_tokens):
        ...
        # elif args.use_mds:
            ...
            if (step == 0):
                hidden_states_new = torch.concat((input_embedding[:, 1:], hidden_states_output), dim=-1)
                small_hidden_states = small_model.fc(hidden_states_new).squeeze(0)
            else:
                hidden_states_new = torch.concat((input_embedding[:, -accept_length:], hidden_states_output), dim=-1)
                small_hidden_states = torch.concat((small_hidden_states, small_model.fc(hidden_states_new).squeeze(0)), dim=0)

            next_step_tensors_s = None
            context_lengths_s = context_lengths_o.clone()
            host_context_lengths_s = context_lengths_s.cpu()
            small_hidden_states_s = small_hidden_states.clone()
            max_context_length_s = small_hidden_states_s.shape[0]

            for idx in range(draft_token_len):
                # print("small_hidden_states_s", small_hidden_states_s)
                # print("context_lengths", context_lengths)
                input_ids_ = torch.arange(small_hidden_states_s.shape[0]).to(torch.int32)
                (should_stop_, next_step_tensors_, tasks_, context_lengths_, host_context_lengths_, attention_mask_, logits_,
                generation_logit_, encoder_input_lengths_, small_hidden_states_) = small.handle_per_step(
                    small_cache_indirections, 0, batch_size, max_context_length_s,
                    beam_width, input_ids_, small_hidden_states_s, scfg,
                    ...
                    **kwargs)
                small_hidden_states_s = torch.concat((small_hidden_states_s, small_hidden_states_), dim=0)
